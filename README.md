...
<h1 align="center">Gald3r for Hermes (Nous Research)</h1>

<p align="center">
  The <strong>Hermes (Nous Research) edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Hermes (Nous Research)</strong>. Part of the Gald3r framework
...
This is the **Hermes (Nous Research) distribution** of Gald3r. The [`hermes/`](./hermes/) folder is a
complete, ready-to-deploy gald3r setup tuned for Hermes (Nous Research) -- drop its contents into your
...
```bash
git clone https://github.com/wrm3/gald3r_platform_hermes.git
cp -r gald3r_platform_hermes/hermes/. /path/to/your/project/
```

Then open the project in Hermes (Nous Research) and type `@g-status` to confirm gald3r loaded.

> The `hermes/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
